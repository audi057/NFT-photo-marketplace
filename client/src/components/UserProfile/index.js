...
export default class UserProfile extends Component {
    ...
    handleStateCategory = (event) => {
      if (event.target.value == 'On sale') {
        this.setState({ valueStatus: "Open" });
      }
      else if(event.target.value == 'Bought') {
        this.setState({ valueStatus: "Cancelled" });
      }
      else {
        this.setState({ valueStatus: 'All' });
      }
    }
    ...
    getPathId() {
      ...
      const firstIndex = pathname.indexOf('/') + 1;
      let lastIndex = pathname.indexOf('/', firstIndex);

      if(lastIndex == -1) lastIndex = pathname.length;

      return pathname.substring(firstIndex, lastIndex);
    }
                ...
                return (
                  <div key={key} className="">
                    <div className={styles.widgets}>
                        ...
                        { (this.state.valueStatus == "All" && this.state.id == photo.ownerAddress) || (this.state.valueStatus == photo.status && this.state.id == photo.ownerAddress) ? 
                            <Card width={"360px"} 
                                    maxWidth={"360px"} 
                                    mx={"auto"} 
                                    my={5} 
                            ...
                            >
                              ...
                              { photo.status == "Cancelled" ? 
                                  <Button size={'medium'} width={1} value={ photo.photoNFT } onClick={() => this.putOnSale(photo.photoNFT)} disabled> Put on sale </Button>
                              :
                                  <Button size={'medium'} width={1} value={ photo.photoNFT } onClick={() => this.cancelOnSale(photo.photoNFT)} disabled> Cancel on sale </Button>
                              }
